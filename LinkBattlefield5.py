...
P_list[32] = [6,90,2,10,0,"主教",2,0]


def filtered_permutations(input_dict):
    # 筛选出value小于10的元素
    filtered_items = [item for item in input_dict.items() if item[1][0] <= 5]
    return list(permutations(filtered_items, 4))
...
def sum_damage(atk_para):
    ...
    if 1 <= atk_para[0] <= 8:
        if atk_para[7] % atk_para[0] == 0:
            ...
            atk_count = atk_count + atk_para[2]
    else:
...
def check_damage(zongshanghai, Bossxueliang, wulishanghai, mofashanghai, zhandouli=300000):
    ...
    Total_Damage = zongshanghai + Damage
    if Damage != 0:
        #print("当前BOSS血量上限",f"{Bossxueliang[minkey][0]:,.0f}","已累计伤害",f"{Total_Damage:,.0f}")
        #print("当前层数", minkey)
        if Bossxueliang[minkey][0]-Total_Damage <=0:
            Total_Damage = 0
            #print("消除Boss，进入下一秒")
